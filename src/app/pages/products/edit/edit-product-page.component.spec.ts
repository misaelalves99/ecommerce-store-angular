...
describe('EditProductPageComponent', () => {
  ...
  const mockCategories: Category[] = [
    { id: 1, name: 'Categoria X', description: 'Desc X', createdAt: '2025-08-23T12:00:00Z' },
    { id: 2, name: 'Categoria Y', description: 'Desc Y', createdAt: '2025-08-23T12:00:00Z' }
  ];
  ...
  beforeEach(async () => {
    ...
    productServiceSpy = jasmine.createSpyObj('ProductService', ['getProducts', 'addProduct']);
    ...
    categoryServiceSpy.getCategories.and.returnValue(mockCategories);
    brandServiceSpy.getBrands.and.returnValue(mockBrands);
  ...
  it('deve redirecionar para /products se o produto não for encontrado', () => {
    ...
    spyOn(window, 'alert'); // evita alerta real
    component.ngOnInit();
  ...
  it('deve chamar ProductService.addProduct e navegar ao salvar', () => {
    ...
    expect(productServiceSpy.addProduct).toHaveBeenCalledWith(updated);
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/products']);
  });

  it('deve navegar de volta para /products ao cancelar', () => {
